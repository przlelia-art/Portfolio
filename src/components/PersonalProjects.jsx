...
// NOUVEAU : Projets thématiques basés sur tes passions
const futureThematicProjects = [
    {
        status: "En préparation",
        title: "MotoGP Data Lab",
        theme: "MotoGP",
        note: "Analyse comparative des styles de pilotage, des données de télémétrie et de l'évolution des chronos en fonction de l'usure des pneus.",
    },
    {
        status: "Idée",
        title: "Choreo-Graph & Rythme",
        theme: "Danse",
        note: "Exploration d'une application interactive autour du tempo, de la structure des chorégraphies ou de la visualisation de mouvements.",
    },
    {
        status: "En préparation",
        title: "Personal Running Hub",
        theme: "Course à pied",
        note: "Création d'un dashboard personnalisé pour suivre mes sessions, analyser mes allures, mes records et visualiser mes progressions de course.",
    },
];
...
function PersonalVisual({ type }) {
    ...
    if (type === "github") {
        ...
        <div style={{
            width: "85%", height: "78%",
            background: "#0d0d12", borderRadius: "8px",
        ...
        <div style={{ padding: "12px", fontFamily: "monospace", fontSize: "10px", color: "#5ce1ff", lineHeight: "1.8" }}>
    ...
    if (type === "cobol") {
        ...
        <div style={{ color: "rgba(255,255,255,0.3)" }}>
            ...
            <span style={{ color: "#ff8a65", fontHex: "bold", fontWeight: "bold" }}>IDENTIFICATION DIVISION.</span>
        </div>
        <div style={{ color: "rgba(255,255,255,0.3)" }}>
            <span style={{ color: "#5ce1ff" }}>000200</span>{" "}
            <span style={{ color: "#ff8a65", fontHex: "bold", fontWeight: "bold" }}>PROGRAM-ID.</span> LEARNING.
        </div>
        <div style={{ color: "rgba(255,255,255,0.3)" }}>
            <span style={{ color: "#5ce1ff" }}>000300</span>{" "}
            <span style={{ color: "#ff8a65", fontHex: "bold", fontWeight: "bold" }}>PROCEDURE DIVISION.</span>
        </div>
        <div style={{ color: "rgba(255,255,255,0.3)" }}>
            <span style={{ color: "#5ce1ff" }}>000400</span>{" "}
            DISPLAY "HELLO WORLD".
        </div>
        </div>
        );
    ...
    if (type === "c") {
        ...
        <div style={{
            width: "85%", height: "78%",
            background: "#0d0d12", borderRadius: "8px",
            border: "1px solid rgba(0,217,255,0.15)", overflow: "hidden",
        }}>
    ...
    return (
        <>
            <style>{`
                    ...
                    .learning-status.idee { color: rgba(255,255,255,0.35); }
                    ...
                    .status-dot.idee { background: rgba(255,255,255,0.3); animation: none; }
                    ...
                    .learning-theme {
                        font-size: 11px; color: #ff8a65; font-weight: 500; margin-bottom: 8px;
                    }
        ...
        <div style={{ maxWidth: "1140px", margin: "0 auto", position: "relative", zIndex: 1 }}>
            ...
            <h2 style={{
                fontFamily: "'Space Grotesk', sans-serif",
                fontSize: "42px", fontWeight: "700", color: "#ffffff",
                ...
                <div className="pp-body">
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "16px" }}>
                        {project.technologies.map((tech, i) => (
                            <span key={i} className="pp-tech">{tech}</span>
                    ))}
                    </div>
                    <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="pp-github">
            ...
            {/* SECTION MIS À JOUR : Prochains défis thématiques */}
            <h2 style={{
                fontFamily: "'Space Grotesk', sans-serif",
                fontSize: "28px", fontWeight: "700", color: "#fff",
                marginBottom: "30px",
            }}>
                Prochains défis & Explorations
            </h2>
            <div
                className="learning-grid"
                style={{ 
                    display: "grid", 
                    gridTemplateColumns: "repeat(3, 1fr)", // Passage à 3 colonnes pour s'aligner sur tes 3 thèmes
                    gap: "16px" 
                }}
            >
                {futureThematicProjects.map((item, i) => (
                <div key={i} className="learning-card">
                    <div className={`learning-status ${item.status === "Idée" ? "idee" : ""}`}>
                        <span className={`status-dot ${item.status === "Idée" ? "idee" : ""}`} />
                        {item.status}
                    </div>
                    <div className="learning-title">{item.title}</div>
                    <div className="learning-theme">Thème : {item.theme}</div>
                    <div className="learning-note">{item.note}</div>
                </div>
            ))}
        </div>
